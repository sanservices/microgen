...
func New(ctx context.Context, config *config.Config, {% if cookiecutter.use_database == 'y' %}db *sqlx.DB,{% endif %} {% if cookiecutter.use_cache == 'y' %}cache *redis.Client{% endif %}) {{ cookiecutter.entity_name }}.Repository {
	return &Repository{
